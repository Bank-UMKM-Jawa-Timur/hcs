...
class LaporanTetapRepository
{
    ...
    public function get($kantor = 'keseluruhan', $search, $limit=10, $cetak, $year){
        ...
        $data = KaryawanModel::with([
                ...
                'tunjanganTidakTetap' => function($query) use ($year) {
                    $query->where('mst_tunjangan.kategori', 'tidak teratur')
                        ->where('penghasilan_tidak_teratur.tahun', $year);
                },
                'bonus' => function($query) use ($year) {
                    $query->where('mst_tunjangan.kategori', 'bonus')
    ...
    public function getTotal($kantor = 'keseluruhan', $search, $limit=10, $cetak, $year){
        $cabangRepo = new CabangRepository;
        $kode_cabang_arr = $cabangRepo->listCabang(true);
        ...
        $returnData->totalTeratur = $totalTeratur;
        $returnData->totalTidakTeratur = $totalTidakTeratur;
        $returnData->totalPenghasilanBruto = $totalPenghasilanBruto;
        $returnData->totalBiayaJabatan = $totalBiayaJabatan;
        $returnData->totalIuranPensiun = $totalIuranPensiun;
        $returnData->totalPenghasilanNeto = $totalPenghasilanNeto;
        $returnData->totalPenghasilanNetoSetahun = $totalPenghasilanNetoSetahun;
        $returnData->totalPtkp = $totalPtkp;
        $returnData->totalPkp = $totalPkp;
        $returnData->totalPph21Terutang = $totalPph21Terutang;
        $returnData->totalPph21Dibayar = $totalPph21Dibayar;
        $returnData->totalKekuranganPph21 = $totalKekuranganPph21;
