...
@section('content')
    <div class="card-header">
        <div class="card-header">
            ...
            <p class="card-title"><a href="">Setting </a> > <a href="{{ route('profil-kantor-pusat.index') }}">Kantor Pusat</a> > <a>Profil</a></p>
        </div>
    </div>
        ...
        <div class="row">
            <div class="col">
                <form action="{{ route('profil-kantor-pusat.update') }}" method="POST" class="form-group">
                    ...
                    <input type="text" class="@error('kode_cabang') is_invalid @enderror form-control" value="{{ old('kode_cabang', $data->kd_cabang) }}" name="kode_cabang" id="kode_cabang" readonly>
                    @error('kode_cabang')
                    ...
                    <input type="email" class="@error('email') is_invalid @enderror form-control" value="{{ old('email', $data->email) }}" name="email" id="email" required>
                    @error('email')
                        <div class="mt-2 alert alert-danger">{{ $message }}</div>
                    @enderror
                    ...
                    @error('nama_pemimpin_cabang')
                        <div class="mt-2 alert alert-danger">{{ $message }}</div>
                    @enderror
                    <button class="is-btn is-primary-light" type="submit">Simpan</button>
                </form>
            </div>
        </div>
    </div>
